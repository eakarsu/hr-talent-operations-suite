const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:4550';
...
const adminCookie = await login('admin@hr-talent.local', 'admin123');
const managerCookie = await login('manager@hr-talent.local', 'manager123');
const analystCookie = await login('analyst@hr-talent.local', 'analyst123');

await expectJson('/api/dashboard', adminCookie);
await expectJson('/api/entities/employees', analystCookie);
await expectJson('/api/documents', analystCookie);
await expectStatus('/api/documents/upload', analystCookie, 405);

const records = await expectJson('/api/entities/employees', managerCookie);
const rowId = records.rows[0].id;

const approveResponse = await fetch(`${baseUrl}/api/entities/employees/approve`, {
  method: 'POST',
  headers: {
...
const forbiddenApprove = await fetch(`${baseUrl}/api/entities/employees/approve`, {
  method: 'POST',
  headers: {
